...
# Invoke tasks - a Python-equivalent of a Makefile or Rakefile.
...
SERVICE_NAME = os.environ.get("SERVICE_NAME", "inferencia-enr-api")
...
def start(c):  # noqa: ANN001, ANN201
    ...
    with c.prefix(venv):
        c.run("uvicorn app:app --host 0.0.0.0 --port 8080")
...
def dev(c):  # noqa: ANN001, ANN201
    ...
    with c.prefix(venv):
        c.run("uvicorn app:app --host 0.0.0.0 --port 8080 --reload")
...
def _determine_local_import_names(start_dir: str) -> List[str]:
    """Determine all import names that should be considered local."""
...
def build(c):  # noqa: ANN001, ANN201
    ...
    c.run(
        f"gcloud builds submit --pack image={REGION}-docker.pkg.dev/"
        f"{GOOGLE_CLOUD_PROJECT}/{REPOSITORY}/{SERVICE_NAME}:manual"
    )
